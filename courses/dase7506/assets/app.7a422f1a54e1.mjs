import {REPO, PROTOCOL, KEY_ID, PROJECTS, safeURL, validScore, validStudentID, makeIssueURL, bestRows, submissionPhase} from './arena.251dee5fbd5c.mjs';
...
$('submission-form').addEventListener('submit', async event => {
  ...
  try {
    ...
    if(phase==='scores') {
      ...
      if(!validStudentID(studentId)) throw new Error('Enter a student ID of 3–32 letters, numbers or hyphens.');
...
$('challenge-form').addEventListener('submit', event => {
  ...
  try {
    ...
    if(!challengeTarget || ['closed','complete'].includes(phase)) throw new Error('Review requests are closed.');
    ...
    if(reason.length<10 || reason.length>2000) throw new Error('Explain your request in 10–2,000 characters.');
    if((rawScore && !validScore(challengeTarget.project,score)) || (rawEvidence && !evidence)) throw new Error('Check the optional score and HTTPS evidence link.');
    const reproduction=phase==='review' && score!==null && !!evidence;
    const threshold = PROJECTS[challengeTarget.project].review_threshold;
    if (reproduction && typeof threshold === 'number' && Math.abs(score-challengeTarget.score) <= threshold) throw new Error(`The difference does not exceed the review threshold of ${threshold} ${PROJECTS[challengeTarget.project].unit}. Discuss general questions on the original submission.`);
    const data = {schema:reproduction?'dase7506/challenge-v1':'dase7506/review-request-v1',protocol:PROTOCOL,project:challengeTarget.project,
