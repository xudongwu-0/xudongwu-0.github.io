...
import {PROTOCOL, PROJECTS} from './projects.c353c6312e62.mjs';
...
export function parseIssue(issue) {
  ...
  try {
    ...
    if (!validateSubmission(d) && !validateChallenge(d) && !validateArtifacts(d)) return null;
...
export function makeIssueURL(data) {
  ...
  if (!submission && !validateChallenge(data) && !artifacts) throw new Error('Invalid submission data.');
  const title = submission
    ? `[DASE7506] ${data.project.toUpperCase()} · ${data.score} ${PROJECTS[data.project].unit}`
    : artifacts ? `[DASE7506 artifacts] #${data.submission}` : `[DASE7506 reproduction] #${data.submission}`;
  const intro = submission
    ? 'Self-reported score. Student ID and any sealed artifact links are encrypted for the instructor. Scores and GitHub usernames are public.\n\nKeep the JSON unchanged. Submit improvements as new issues before the score deadline.'
...
export function buildSnapshot(issues, previous={}, now=new Date().toISOString(), publication={projects:{}}) {
