import {REPO, PROTOCOL, KEY_ID, PROJECTS, safeURL, validScore, makeIssueURL, bestRows, submissionPhase} from './arena.56adbdba2c71.mjs';
...
function render() {
  ...
  rows.forEach((row, i) => {
    ...
    button.type = 'button'; button.disabled = !row.code_url || !row.checkpoint_url || submissionPhase(row.project,snapshot.publication)!=='review'; button.addEventListener('click', () => openChallenge(row)); reproduce.append(button);
...
async function encryptStudentID(studentId) {
  const ciphertext=await crypto.subtle.encrypt({name:'RSA-OAEP'},await loadPublicKey(),new TextEncoder().encode(studentId));
  return {algorithm:'RSA-OAEP-256',key_id:KEY_ID,ciphertext:base64(ciphertext)};
}
...
$('submission-form').addEventListener('submit', async event => {
  ...
  try {
    ...
    if(phase==='scores') {
      ...
      if(!/^[A-Za-z0-9-]{3,32}$/.test(studentId)) throw new Error('Enter a student ID of 3–32 letters, numbers or hyphens.');
      data={schema:'dase7506/submission-v1',protocol:PROTOCOL,project:p,score,student_id:await encryptStudentID(studentId)};
...
$('challenge-form').addEventListener('submit', event => {
  ...
  try {
    if(!challengeTarget || submissionPhase(challengeTarget.project,snapshot.publication)!=='review') throw new Error('Peer review opens after the instructor releases links, and lasts seven days.');
